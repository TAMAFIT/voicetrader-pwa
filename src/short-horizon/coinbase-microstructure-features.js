...
export function buildCoinbaseBookMicrostructureFeature({productId,previous,current,integrityEvidence,receivedTimestampMs,sourceSha256,sequence}={}){
  if(!allowedProducts.has(productId)||!['TRUSTED_SNAPSHOT','TRUSTED_UPDATE'].includes(integrityEvidence?.status)||integrityEvidence?.semantics?.localBookSynchronizationVerified!==true||integrityEvidence?.semantics?.derivedMicrostructureAuthorized!==true||integrityEvidence?.governance?.derivedMicrostructureAuthorized!==true)return null;
