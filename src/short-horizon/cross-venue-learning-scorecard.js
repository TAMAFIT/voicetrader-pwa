...
export function updateCrossVenueLearningScorecard(state,observation){
  ...
  const f=Number(observation.replication.featureSignAgreementRate);if(Number.isFinite(f)){g.featureRateCount+=1;g.featureRateSum+=f;}const df=Number(observation.replication.directionalFeatureSignAgreementRate);if(Number.isFinite(df)){g.directionalFeatureRateCount+=1;g.directionalFeatureRateSum+=df;}return state;
}
...
function diff(a,b){return Number.isFinite(Number(a))&&Number.isFinite(Number(b))?round(Number(a)-Number(b),6):null;}
