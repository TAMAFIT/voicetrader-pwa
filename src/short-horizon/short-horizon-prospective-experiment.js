...
export const FROZEN_EXPERIMENTS=Object.freeze([
  Object.freeze({experimentId:'BOUNDARY_CONCORDANCE_V1',sampleRole:'PRIMARY',inputWindowSec:5,horizonsSec:Object.freeze([5,15,30,60]),eligibilityMode:'BOUNDARY',boundaryBeforeSec:10,boundaryAfterSec:5,minVotes:3,description:'Boundary-conditioned concordance across trusted OFI, L2 imbalance, microprice and taker flow.'}),
  Object.freeze({experimentId:'BOUNDARY_HASH_NULL_V1',sampleRole:'NULL',inputWindowSec:5,horizonsSec:Object.freeze([5,15,30,60]),eligibilityMode:'BOUNDARY',boundaryBeforeSec:10,boundaryAfterSec:5,description:'Same eligible boundary windows, deterministic future-independent hash direction null.'}),
  Object.freeze({experimentId:'PHASE_CONTROL_CONCORDANCE_V1',sampleRole:'PHASE_CONTROL',inputWindowSec:5,horizonsSec:Object.freeze([5,15,30,60]),eligibilityMode:'PHASE_CONTROL',phaseControlOffsetsSec:Object.freeze([140,145,150,155]),minVotes:3,description:'Same concordance rule sampled at preregistered interior 5-minute phase offsets, away from the boundary.'}),
]);
...
export function boundaryEligibility(window,spec){
  const boundaries=window?.timing?.boundary||{};
  if(spec.eligibilityMode==='PHASE_CONTROL'){const since=Number(boundaries?.['5m']?.secondsSinceBoundary);const offsets=spec.phaseControlOffsetsSec||[];const eligible=Number.isFinite(since)&&offsets.includes(since);return {eligible,matchedBoundaries:eligible?['5m-phase-control']:[]};}
  const matched=[];
