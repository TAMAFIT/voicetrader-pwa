...
export const MODEL_EXPERIMENT_LIFECYCLE_VERSION='model-experiment-lifecycle-ledger-0.1';
...
export function createModelExperimentLifecycleDocument({updatedAt=new Date().toISOString()}={}){
  const document={schemaVersion:MODEL_EXPERIMENT_LIFECYCLE_VERSION,updatedAt,frozenSpecs:[],events:[],governance:{...MODEL_EXPERIMENT_LIFECYCLE_GOVERNANCE}};
...
export function auditModelExperimentLifecycleDocument(document){
  const errors=[],states={},frozenByKey=new Map();
  if(document?.schemaVersion!==MODEL_EXPERIMENT_LIFECYCLE_VERSION)errors.push('schema-version-mismatch');
  const frozenSpecs=Array.isArray(document?.frozenSpecs)?document.frozenSpecs:[];const events=Array.isArray(document?.events)?document.events:[];
  if(!Array.isArray(document?.frozenSpecs))errors.push('frozen-specs-not-array');if(!Array.isArray(document?.events))errors.push('events-not-array');
  ...
  for(const event of events){const key=keyOf(event?.experimentId,event?.revision);const time=Date.parse(event?.occurredAt||'');if(!Number.isFinite(time))errors.push(`event-time-invalid:${event?.eventId||key}`);else if(time<priorTime)errors.push(`event-order-invalid:${event?.eventId||key}`);else priorTime=time;
    ...
    const state=states[key]||{experimentId:event?.experimentId,revision:event?.revision,effectiveStatus:null,draftRegistered:false,frozen:false,retired:false,semanticFingerprint:null};
    if(event?.type==='DRAFT_REGISTERED'){if(state.draftRegistered||state.frozen||state.retired)errors.push(`illegal-draft-transition:${key}`);if(!String(event?.actor||'').trim())errors.push(`draft-actor-missing:${key}`);if(!/^fnv1a64:[0-9a-f]{16}$/.test(String(event?.draftFingerprint||'')))errors.push(`draft-fingerprint-invalid:${key}`);state.draftRegistered=true;state.effectiveStatus='DRAFT';}
    else if(event?.type==='FROZEN'){const spec=frozenByKey.get(key);if(!state.draftRegistered)errors.push(`freeze-without-draft:${key}`);if(state.frozen||state.retired)errors.push(`duplicate-or-late-freeze:${key}`);if(!spec)errors.push(`freeze-spec-missing:${key}`);else {if(event.semanticFingerprint!==spec.semanticFingerprint)errors.push(`freeze-fingerprint-drift:${key}`);if(event.datasetCommit!==spec.dataset.commitSha||Number(event.datasetCutoffTime)!==Number(spec.dataset.cutoffTime))errors.push(`freeze-dataset-drift:${key}`);if(event.codeCommit!==spec.environment.codeCommit)errors.push(`freeze-code-drift:${key}`);if(event.approvalActor!==spec.approval.actor||event.approvalTime!==spec.approval.approvedAt||event.actor!==spec.approval.actor||event.occurredAt!==spec.approval.approvedAt)errors.push(`freeze-approval-drift:${key}`);}state.frozen=true;state.effectiveStatus='FROZEN';state.semanticFingerprint=event.semanticFingerprint;}
  ...
  return {version:MODEL_EXPERIMENT_LIFECYCLE_AUDIT_VERSION,status:errors.length?'fail':'pass',pass:errors.length===0,errorCount:errors.length,errorCodes:[...new Set(errors)],checked:{frozenSpecs:frozenSpecs.length,events:events.length},counts,effectiveStates};
}

export function appendModelExperimentLifecycleEvent(document,event,{frozenSpec=null,updatedAt=event?.occurredAt||new Date().toISOString()}={}){
  const before=auditModelExperimentLifecycleDocument(document);if(!before.pass)throw new Error(`lifecycle-document-invalid:${before.errorCodes.join(',')}`);const next=clone(document);next.events.push(clone(event));if(event?.type==='FROZEN'){if(!frozenSpec)throw new Error('frozen-spec-required');const validation=validateModelExperimentSpec(frozenSpec,{requireFrozen:true});if(!validation.pass)throw new Error(`invalid-frozen-spec:${validation.errorCodes.join(',')}`);next.frozenSpecs.push(clone(frozenSpec));}next.updatedAt=updatedAt;next.audit=auditModelExperimentLifecycleDocument(next);if(!next.audit.pass)throw new Error(`lifecycle-append-rejected:${next.audit.errorCodes.join(',')}`);return next;
}
