...
export const MODEL_EXPERIMENT_SPEC_CONTRACT_VERSION='model-experiment-spec-contract-0.4';
...
export function validateModelExperimentSpec(spec,{requireFrozen=false}={}){
  ...
  return {version:'model-experiment-spec-validation-0.4',pass:errors.length===0,errorCount:errors.length,errorCodes:[...new Set(errors)]};
}
...
export function evaluateModelExperimentWorkerEligibility({spec,readiness}={}){const validation=validateModelExperimentSpec(spec,{requireFrozen:true});const instruments=spec?.scope?.instruments||[];const requiresCrossMarket=instruments.includes('ETHUSD');const dataReady=requiresCrossMarket?readiness?.gates?.crossMarketResearchModelReady===true:readiness?.gates?.btcResearchModelReady===true;const eligible=validation.pass&&dataReady&&spec?.approval?.humanApproved===true;return {version:'model-experiment-worker-eligibility-0.4',eligibleForApprovedWorker:eligible,specValid:validation.pass,dataReady,humanApproved:spec?.approval?.humanApproved===true,readinessGate:requiresCrossMarket?'cross-market':'btc',registryLaunchesJobs:false,trainingImplemented:false,automaticTraining:false,automaticPromotion:false,reasons:[...(!validation.pass?validation.errorCodes:[]),...(!dataReady?['learning-readiness-gate-not-ready']:[]),...(spec?.approval?.humanApproved!==true?['human-approval-missing']:[])]};}
