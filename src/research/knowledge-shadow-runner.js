import { HumanKnowledgeEngine, HUMAN_KNOWLEDGE_ENGINE_VERSION } from '../knowledge/human-knowledge-engine.js';

export const KNOWLEDGE_SHADOW_VERSION = 'knowledge-shadow-0.1';
export const KNOWLEDGE_SHADOW_HORIZON_BARS = 3;
export const KNOWLEDGE_SHADOW_START_INDEX = 100;
...
function returnsBps(series, entryIndex, exitIndex, side, costBps) {
...
function summarize(trades, startIndex, endIndex) {
  let equity = 1;
  let peak = 1;
  let maxDrawdown = 0;
  let wins = 0;
  ...
  return {
    ...
    exposurePct: round(Math.min(100, count * KNOWLEDGE_SHADOW_HORIZON_BARS / observedBars * 100), 1),
    regimeCounts,
  };
}

export function runKnowledgeShadow({
  series,
  endIndex,
  ...
  if (!Array.isArray(series) || series.length <= startIndex + horizonBars) {
    return { version:KNOWLEDGE_SHADOW_VERSION, status:'unavailable', reason:'insufficient-series', trades:[] };
  }
  ...
  for (let idx = startIndex; idx + horizonBars <= safeEnd; idx++) {
    ...
    const side = sideFromDecision(analysis.entryDecision);
    if (!side) continue;
    const exitIndex = idx + horizonBars;
    const outcome = returnsBps(series, idx, exitIndex, side, estimatedRoundTripCostBps);
    if (!outcome) continue;
    trades.push({
      entryIndex: idx,
      exitIndex,
      ...
      riskGate: analysis.context.riskGate,
      knowledgeScore: analysis.knowledgeScore,
      familyAgreement: analysis.familyAgreement,
      familyScores: Object.fromEntries(Object.entries(analysis.families).map(([key, value]) => [key, value.score])),
      ...outcome,
    });
  ...
  if (JSON.stringify(series) !== sourceBefore) {
    return { version:KNOWLEDGE_SHADOW_VERSION, status:'blocked', reason:'source-series-mutated', trades:[] };
  }

  return {
    version: KNOWLEDGE_SHADOW_VERSION,
    engineVersion: HUMAN_KNOWLEDGE_ENGINE_VERSION,
    status: 'complete',
    dataSignature,
    startIndex,
    endIndex: safeEnd,
    ...
    decisionCounts,
    summary: summarize(trades, startIndex, safeEnd),
    trades,
    methodology: {
      purpose: 'research-only-human-knowledge-shadow',
      fixedHorizonBars: horizonBars,
      nonOverlappingTrades: true,
      familyNormalized: true,
