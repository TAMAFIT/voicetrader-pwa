...
export const RESEARCH_EXPORT_VERSION='research-export-0.10';
export const PREVIOUS_RESEARCH_EXPORT_VERSION='research-export-0.9';
...
export function buildResearchJson({events=[],baselineEvaluation=null,strategyRegistry=null,challengerEvaluation=null,walkForwardEvaluation=undefined,forwardDemoEvaluation=undefined,knowledgeEvaluation=undefined,playbookEvaluation=undefined,knowledgeCandidateTournament=undefined,knowledgeForwardEvaluation=undefined,nullMarketEvaluation=null,dataMeta=null}={}){const resolvedWalk=walkForwardEvaluation===undefined?getLatestWalkForwardEvaluation():walkForwardEvaluation;const resolvedForward=forwardDemoEvaluation===undefined?getLatestForwardDemoEvaluation():forwardDemoEvaluation;const resolvedKnowledge=knowledgeEvaluation===undefined?getLatestKnowledgeEvaluation():knowledgeEvaluation;const resolvedPlaybook=playbookEvaluation===undefined?getLatestPlaybookEvaluation():playbookEvaluation;const resolvedTournament=knowledgeCandidateTournament===undefined?getLatestKnowledgeCandidateTournament():knowledgeCandidateTournament;const resolvedKnowledgeForward=knowledgeForwardEvaluation===undefined?getLatestKnowledgeForwardEvaluation():knowledgeForwardEvaluation;return JSON.stringify({exportVersion:RESEARCH_EXPORT_VERSION,exportedAt:new Date().toISOString(),eventCount:events.length,notes:[
...
'Knowledge prospective evidence is currently browser-local. Clearing site storage can erase it, so exported JSON should be retained until autonomous durable collection is implemented.',
'Existing Prospective Forward Demo forward-001 remains separate and unchanged.',
'No same-series, holdout, or prospective screen automatically promotes a Champion; negative-control review, execution review and human approval remain required.',
'Null Market / Negative Control results are screening diagnostics only; Null95 and exceedance rates are not formal p-values.',
'Human Knowledge, Attribution, Playbook, Candidate Tournament, knowledge-forward-001, Challenger, walk-forward and Null outputs are never inputs to frozen live/demo Champion decisions.',
'Synthetic market data is not research eligible.'
],dataMeta,baselineEvaluation,strategyRegistry,challengerEvaluation,knowledgeEvaluation:resolvedKnowledge,playbookEvaluation:resolvedPlaybook,knowledgeCandidateTournament:resolvedTournament,knowledgeForwardEvaluation:resolvedKnowledgeForward,walkForwardEvaluation:resolvedWalk,forwardDemoEvaluation:resolvedForward,nullMarketEvaluation,decisionEvents:events},null,2);}
