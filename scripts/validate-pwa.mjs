...
for(const marker of ['VoiceTrader','VoiceTrader Demo v0.3 UI','id="chartCanvas"','id="scannerGrid"','id="buyBtn"','id="sellBtn"','id="humanEquity"','id="aiEquity"','class="duel-strip','class="decision-card']){if(!html.includes(marker))fail(`index.html missing marker: ${marker}`)}
...
for(const file of ['index.html','styles.css','manifest.webmanifest','src/app.js','src/config.js','src/pwa.js','src/engine/indicators.js','src/engine/shadow-engine.js','src/engine/execution-engine.js','src/engine/ai-provider.js','assets/icons/icon-192.png','assets/icons/icon-512.png','assets/icons/icon-maskable-512.png']){if(!sw.includes(`./${file}`))fail(`service worker cache missing: ${file}`)}
if(!process.exitCode)console.log('PWA v0.3 UI integrity validation passed.');
