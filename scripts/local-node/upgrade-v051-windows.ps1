param(
  ...
  [string]$RepoRef = 'cfe802d748bc80fcd1e5903d307b262b1bf96f97'
)
...
$derivedWorker=Join-Path $scriptDir 'gmo-quote-derived-worker.mjs'
$krakenRecorder=Join-Path $scriptDir 'kraken-microstructure-recorder.mjs'
& $nodeExe --check $derivedWorker
if($LASTEXITCODE -ne 0){throw 'Derived worker syntax check failed.'}
& $nodeExe --check $krakenRecorder
if($LASTEXITCODE -ne 0){throw 'Kraken recorder syntax check failed.'}
...
Register-VoiceTraderTask $DerivedTask ('"{0}" --root "{1}" --poll-ms 15000 --lookback-minutes 5' -f $derivedWorker,$DataRoot) 'VoiceTrader local-only USDJPY derived quote engine. Reads local raw files only.'
Register-VoiceTraderTask $KrakenTask ('"{0}" --root "{1}" --warn-free-gb 50 --hard-stop-free-gb 10' -f $krakenRecorder,$DataRoot) 'VoiceTrader public Kraken BTC/ETH trade + L2 raw recorder. No authentication or order routing.'

$config=[ordered]@{
  schemaVersion='voicetrader-local-upgrade-v051'
  ...
  components=@(
    ...
    [ordered]@{id='kraken-raw';task=$KrakenTask;endpoint='wss://ws.kraken.com/v2';symbols=@('BTC/USD','ETH/USD');channels=@('book','trade');bookDepth=10;authenticationRequired=$false}
  )
  runtimePolicy=[ordered]@{googleCloudEnabled=$false;cloudUploadEnabled=$false;githubActionsRequired=$false;orderSubmission=$false;realMoneyRouting=$false}
  integrity=[ordered]@{krakenExactWirePreserved=$true;krakenPerMessageSha256=$true;krakenChecksumVerification='DEFERRED_TO_V052';ofiAvailable=$false;micropriceAvailable=$false}
}
$config|ConvertTo-Json -Depth 8|Set-Content -Encoding UTF8 -Path (Join-Path $DataRoot 'state\local-node-v051-config.json')

Start-Sleep -Seconds 8
Write-Host ''
Write-Host 'VoiceTrader Local Node upgraded through v0.51.'
Write-Host "Data root: $DataRoot"
Write-Host 'Google Cloud = OFF / Cloud upload = OFF / GitHub Actions runtime dependency = NONE'
Write-Host 'GMO Derived: 1s / 5s / 1m local-only features'
Write-Host 'Kraken Raw: BTC/USD + ETH/USD public Trade + L2 Book depth 10'
Write-Host 'Kraken raw wire is preserved exactly; OFI remains disabled until checksum verification in v0.52.'
$derivedHealth=Join-Path $DataRoot 'state\derived-gmo-health.json'
$krakenHealth=Join-Path $DataRoot 'state\kraken-microstructure-health.json'
if(Test-Path $derivedHealth){$h=Get-Content -Raw $derivedHealth|ConvertFrom-Json;Write-Host "Derived worker: $($h.status)"}
if(Test-Path $krakenHealth){$h=Get-Content -Raw $krakenHealth|ConvertFrom-Json;Write-Host "Kraken worker: $($h.status) / messages=$($h.counts.messages) / book=$($h.counts.book) / trade=$($h.counts.trade) / freeGB=$([math]::Round($h.storage.freeBytes/1GB,2))"}else{Write-Host 'Kraken health file not visible yet; task may still be connecting.'}
