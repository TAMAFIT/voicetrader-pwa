param(
  ...
  [string]$RepoRef = 'cc9626813c1520d88635ef97504e2f40bf5decfb'
)
...
$downloads=@(
  ...
  @{Url="$RepoBase/src/short-horizon/kraken-microstructure-features.js";Destination=(Join-Path $sourceDir 'kraken-microstructure-features.js')}
)
...
Register-VoiceTraderTask $KrakenTask ('"{0}" --root "{1}" --warn-free-gb 50 --hard-stop-free-gb 10' -f $krakenRecorder,$DataRoot) 'VoiceTrader public Kraken BTC/ETH trade + L2 recorder with checksum-gated OFI/microprice derivation.'

$config=[ordered]@{schemaVersion='voicetrader-local-upgrade-v053';installedAt=(Get-Date).ToUniversalTime().ToString('o');dataRoot=$DataRoot;repo='TAMAFIT/voicetrader-pwa';repoRef=$RepoRef;nodeVersion=$nodeVersionText;components=@(
  [ordered]@{id='gmo-derived';task=$DerivedTask;intervals=@('1s','5s','1m');runtimeNetwork='NONE';features=@('bidAskMidOHLC','spreadStats','quoteDirection','latency','5m15m60mBoundaries')},
  [ordered]@{id='kraken-microstructure';task=$KrakenTask;endpoint='wss://ws.kraken.com/v2';symbols=@('BTC/USD','ETH/USD');channels=@('book','trade');bookDepth=10;checksum='CRC32_TOP10';derived=@('OFI','microprice','top1Imbalance','depth10Imbalance','signedTakerFlow');authenticationRequired=$false}
);runtimePolicy=[ordered]@{googleCloudEnabled=$false;cloudUploadEnabled=$false;githubActionsRequired=$false;orderSubmission=$false;realMoneyRouting=$false};research=[ordered]@{rawIsAuthoritative=$true;checksumGateRequired=$true;predictionInputAuthorized=$false;automaticPromotion=$false}}
$config|ConvertTo-Json -Depth 8|Set-Content -Encoding UTF8 -Path (Join-Path $DataRoot 'state\local-node-v053-config.json')

Start-Sleep -Seconds 12
Write-Host '';Write-Host 'VoiceTrader Local Node upgraded through v0.53.';Write-Host "Data root: $DataRoot";Write-Host 'Google Cloud = OFF / Cloud upload = OFF / GitHub Actions runtime dependency = NONE';Write-Host 'USDJPY: continuous quote Raw + 1s/5s/1m derived features when FX is open';Write-Host 'BTC/ETH: exact Kraken Raw + Trade + L2 + CRC32 + OFI + Microprice + Book Imbalance + Signed Taker Flow';Write-Host 'Research gate: features are observational only; not yet injected into the frozen prediction model.'
