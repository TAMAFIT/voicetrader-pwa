...
export const COINBASE_LOCAL_RECORDER_VERSION='coinbase-local-public-wire-recorder-v4';
const GiB=1024**3;
...
export function evaluateCoinbaseProviderSequence(previous,current){
  ...
  return {status:'GAP',verified:false,previous:prev,current:cur,reconnectRequired:true,missingFrom:prev+1,missingTo:cur-1};
}
...
export async function runCoinbaseRecorder({rootDir,WebSocketImpl=globalThis.WebSocket,warnFreeBytes=50*GiB,hardStopFreeBytes=10*GiB,now=()=>Date.now(),sleep=(ms)=>new Promise(r=>setTimeout(r,ms)),stopSignal=()=>false}={}){
  ...
  const startedAtMs=now();let sequence=0,reconnects=0,backoffMs=1000,lastMessageAtMs=null,lastLevel2AtMs=null,lastTradeAtMs=null,lastHeartbeatAtMs=null,lastConnectionId=null,lastHealthWrite=0,bookState={},providerSequenceState={status:'UNOBSERVED',verified:false,previous:null,current:null,messagesObserved:0};
  const counts={messages:0,level2:0,marketTrades:0,heartbeats:0,subscriptions:0,parseErrors:0,bytes:0,trustedSnapshots:0,trustedUpdates:0,sequenceGaps:0,sequenceOutOfOrder:0,sequenceInvalid:0,integrityBlocks:0,bookFeatures:0,bookOfiFeatures:0,tradeFeatures:0};
  const snapshotHealth=(status,error=null)=>({schemaVersion:'voicetrader-coinbase-local-health-v4',status,recorderVersion:COINBASE_LOCAL_RECORDER_VERSION,startedAtMs,updatedAtMs:now(),rootDir:path.resolve(rootDir),endpoint:COINBASE_ADVANCED_TRADE_WS_URL,subscriptions:{products:[...COINBASE_LOCAL_PRODUCTS],channels:['level2','market_trades','heartbeats']},connection:{lastConnectionId,reconnects,lastMessageAtMs,lastLevel2AtMs,lastTradeAtMs,lastHeartbeatAtMs,backoffMs,providerSequence:{...providerSequenceState,scope:'FULL_CONNECTION_RAW_STREAM',channels:['subscriptions','level2','market_trades','heartbeats']}},counts:{...counts},storage:{freeBytes:freeBytes(rootDir),warnFreeBytes,hardStopFreeBytes,wireFormat:'RFC7464_JSON_TEXT_SEQUENCE_PLUS_NDJSON_META',hourlyPartition:true},integrity:{exactProviderTextPreserved:true,perMessageSha256:true,providerSequenceContinuityVerified:providerSequenceState.verified===true&&providerSequenceState.messagesObserved>=2,providerSequenceScope:'FULL_CONNECTION_RAW_STREAM',orderBookSynchronizationVerified:COINBASE_LOCAL_PRODUCTS.every((product)=>bookState?.[product]?.trusted===true),bookState},semantics:{derivedFeaturesAvailable:counts.bookFeatures>0||counts.tradeFeatures>0,ofiAvailable:counts.bookOfiFeatures>0,signedTakerFlowAvailable:counts.tradeFeatures>0,crossVenueComparabilityClaim:false,predictionInputAuthorized:false},runtimePolicy:{googleCloudEnabled:false,cloudUploadEnabled:false,githubActionsRequired:false,authenticationRequired:false,orderSubmission:false,realMoneyRouting:false},error:error?String(error):null});
  ...
  for(;;){
    ...
    const connectionId=crypto.randomUUID();lastConnectionId=connectionId;let terminalError=null,closed=false;bookState={};providerSequenceState={status:'UNOBSERVED',verified:false,previous:null,current:null,messagesObserved:0};
    try{
      await new Promise((resolve,reject)=>{
        const ws=new WebSocketImpl(COINBASE_ADVANCED_TRADE_WS_URL),bookTracker=new CoinbaseBookIntegrityTracker({depth:10});
        let lastProviderSequenceNum=null;
        ...
        on('message',(event)=>{const received=now(),raw=typeof event?.data==='string'?event.data:String(event?.data??'');if(!raw)return;sequence++;const persisted=persistCoinbaseWireMessage({rootDir,rawText:raw,receivedTimestampMs:received,connectionId,sequence});counts.messages++;counts.bytes+=persisted.meta.byteLength;if(!persisted.meta.parseOk){counts.parseErrors++;terminalError=new Error('coinbase-provider-json-malformed');finish(terminalError);return;}
          const providerSequence=evaluateCoinbaseProviderSequence(lastProviderSequenceNum,persisted.meta.providerSequenceNum);providerSequenceState={...providerSequence,messagesObserved:Number(providerSequenceState.messagesObserved||0)+1};if(providerSequence.reconnectRequired){if(providerSequence.status==='GAP')counts.sequenceGaps++;else if(providerSequence.status==='OUT_OF_ORDER')counts.sequenceOutOfOrder++;else counts.sequenceInvalid++;appendLog(rootDir,{atMs:received,type:'PROVIDER_SEQUENCE_BLOCK',connectionId,channel:persisted.meta.channel,status:providerSequence.status,previous:providerSequence.previous,current:providerSequence.current,missingFrom:providerSequence.missingFrom??null,missingTo:providerSequence.missingTo??null});terminalError=new Error(`coinbase-provider-sequence:${providerSequence.status}`);finish(terminalError);return;}lastProviderSequenceNum=providerSequence.current;
          if(persisted.meta.isLevel2){counts.level2++;lastLevel2AtMs=received;const previousBooks=bookTracker.snapshot();const integrity=bookTracker.applyRawMessage(raw,{receivedTimestampMs:received,sourceSha256:persisted.meta.sourceSha256,connectionId,providerSequence});for(const item of integrity){appendIntegrity(rootDir,item);if(item.status==='TRUSTED_SNAPSHOT')counts.trustedSnapshots++;else if(item.status==='TRUSTED_UPDATE')counts.trustedUpdates++;else counts.integrityBlocks++;}bookState=bookTracker.snapshot();for(const item of integrity){if(!['TRUSTED_SNAPSHOT','TRUSTED_UPDATE'].includes(item.status))continue;const feature=buildCoinbaseBookMicrostructureFeature({productId:item.productId,previous:previousBooks[item.productId],current:bookState[item.productId],integrityEvidence:item,receivedTimestampMs:received,sourceSha256:persisted.meta.sourceSha256,sequence});if(feature){appendMicrostructure(rootDir,feature);counts.bookFeatures++;if(feature.book.ofi!=null)counts.bookOfiFeatures++;}}const blocked=integrity.find((item)=>item.reconnectRequired===true);if(blocked){terminalError=new Error(`coinbase-book-integrity:${blocked.status}`);finish(terminalError);return;}}
