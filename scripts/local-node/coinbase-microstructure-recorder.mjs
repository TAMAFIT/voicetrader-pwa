...
export const COINBASE_LOCAL_RECORDER_VERSION='coinbase-local-public-wire-recorder-v2';
const GiB=1024**3;
...
function integrityPath(rootDir,timestampMs){const d=new Date(Number(timestampMs)),pad=(v)=>String(v).padStart(2,'0');return path.join(path.resolve(rootDir),'derived','coinbase','book-integrity',String(d.getUTCFullYear()),pad(d.getUTCMonth()+1),pad(d.getUTCDate()),`${pad(d.getUTCHours())}.ndjson`);}
    ...
    try{
      await new Promise((resolve,reject)=>{
        ...
        on('message',(event)=>{const received=now(),raw=typeof event?.data==='string'?event.data:String(event?.data??'');if(!raw)return;sequence++;const persisted=persistCoinbaseWireMessage({rootDir,rawText:raw,receivedTimestampMs:received,connectionId,sequence});counts.messages++;counts.bytes+=persisted.meta.byteLength;if(!persisted.meta.parseOk)counts.parseErrors++;
          if(persisted.meta.isLevel2){counts.level2++;lastLevel2AtMs=received;const integrity=bookTracker.applyRawMessage(raw,{receivedTimestampMs:received,sourceSha256:persisted.meta.sourceSha256,connectionId});for(const item of integrity){appendIntegrity(rootDir,item);if(item.status==='TRUSTED_SNAPSHOT')counts.trustedSnapshots++;else if(item.status==='TRUSTED_UPDATE')counts.trustedUpdates++;else{counts.integrityBlocks++;if(item.status==='SEQUENCE_GAP')counts.sequenceGaps++;if(item.status==='SEQUENCE_OUT_OF_ORDER')counts.sequenceOutOfOrder++;}}bookState=bookTracker.snapshot();const blocked=integrity.find((item)=>item.reconnectRequired===true);if(blocked){terminalError=new Error(`coinbase-book-integrity:${blocked.status}`);finish(terminalError);return;}}
