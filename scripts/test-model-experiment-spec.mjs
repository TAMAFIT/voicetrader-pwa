...
import { MODEL_EXPERIMENT_REGISTRY, MODEL_FEATURE_SET_REGISTRY, MODEL_PREPROCESSING_REGISTRY, MODEL_EXPERIMENT_TEMPLATES } from '../src/research/model-experiment-registry.js';
import {
...
const preprocessing=MODEL_PREPROCESSING_REGISTRY['fixed-tabular-standardize-v3'];assert.equal(MODEL_EXPERIMENT_REGISTRY.frozenSpecs.length,0);assert.equal(MODEL_EXPERIMENT_REGISTRY.governance.trainingImplemented,false);assert.equal(MODEL_EXPERIMENT_REGISTRY.governance.registryLaunchesJobs,false);assert.equal(MODEL_EXPERIMENT_REGISTRY.governance.preprocessingFromOutcomes,false);assert.ok(MODEL_EXPERIMENT_TEMPLATES.every(item=>item.executable===false));assert.equal(preprocessing.numeric.fitPartition,'train-only');assert.equal(preprocessing.numeric.statisticComputation,'sequential-two-pass');assert.equal(preprocessing.numeric.varianceDefinition,'population');assert.equal(preprocessing.numeric.ddof,0);assert.equal(preprocessing.numeric.roundingAlgorithm,'ecmascript-number-tofixed-v1');assert.equal(preprocessing.numeric.roundingTieRule,'nearest-scaled-integer-ties-to-larger-magnitude');assert.equal(preprocessing.numeric.statisticRoundDecimals,12);assert.equal(preprocessing.numeric.transformRoundDecimals,12);assert.equal(preprocessing.categorical.fitCategories,false);

const draft=createModelExperimentDraft({experimentId:'btc-dir-001',templateId:'btc-direction-baseline-template-v1',datasetCommit:'a'.repeat(40),datasetCutoffTime:1788000000,codeCommit:'b'.repeat(40),hypothesis:'Frozen BTC decision-time context may contain incremental 3-bar directional information beyond mandatory baselines.'});draft.createdAt='2026-11-17T00:00:00Z';const draftValidation=validateModelExperimentSpec(draft);assert.equal(draftValidation.pass,true,JSON.stringify(draftValidation.errorCodes));assert.equal(draft.approval.humanApproved,false);assert.equal(draft.governance.automaticTraining,false);assert.equal(draft.algorithm.hyperparameterSearch,false);assert.equal(draft.preprocessing.id,'fixed-tabular-standardize-v3');assert.equal(draft.preprocessing.missingValuePolicy,'reject');assert.equal(draft.preprocessing.unknownCategoryPolicy,'reject');
...
const frozen=freezeModelExperimentSpec(draft,{actor:'human-reviewer',approvedAt:'2026-11-17T01:00:00Z'});const frozenValidation=validateModelExperimentSpec(frozen,{requireFrozen:true});assert.equal(frozenValidation.pass,true,JSON.stringify(frozenValidation.errorCodes));assert.equal(frozen.status,'FROZEN');assert.equal(frozen.approval.humanApproved,true);assert.equal(Object.isFrozen(frozen),true);assert.equal(Object.isFrozen(frozen.dataset),true);assert.equal(Object.isFrozen(frozen.featureSet),true);assert.equal(Object.isFrozen(frozen.featureSet.paths),true);assert.equal(Object.isFrozen(frozen.preprocessing),true);assert.equal(Object.isFrozen(frozen.algorithm.hyperparameters),true);assert.equal(frozen.semanticFingerprint,fingerprintModelExperimentSpec(frozen));assert.equal(compareFrozenModelExperimentSpecs(frozen,structuredClone(frozen)).status,'identical');
...
const preprocessingDrift=structuredClone(draft);preprocessingDrift.preprocessing.numeric.fitPartition='all-data';const preprocessingValidation=validateModelExperimentSpec(preprocessingDrift);assert.ok(preprocessingValidation.errorCodes.includes('preprocessing-contract-drift'));
const numericDrift=structuredClone(draft);numericDrift.preprocessing.numeric.roundingAlgorithm='python-round';const numericValidation=validateModelExperimentSpec(numericDrift);assert.ok(numericValidation.errorCodes.includes('preprocessing-contract-drift'));
