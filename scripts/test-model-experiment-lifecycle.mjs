import './test-model-experiment-control.mjs';
...
import {
  ...
  frozenSpecMatchesRegisteredDraft,
...
assert.equal(document.audit.pass,true);assert.deepEqual(document.audit.counts,{draft:0,frozen:0,retired:0});assert.equal(document.draftSpecs.length,0);assert.equal(document.governance.trainingImplemented,false);assert.equal(document.governance.ledgerLaunchesJobs,false);assert.equal(document.governance.controlledWriterRequired,true);assert.equal(document.governance.workflowDispatchOnly,true);assert.deepEqual(document.governance,MODEL_EXPERIMENT_LIFECYCLE_GOVERNANCE);

const migrated=migrateLegacyEmptyModelExperimentLifecycleDocument({schemaVersion:'model-experiment-lifecycle-ledger-0.1',updatedAt:'2026-11-16T00:00:00Z',frozenSpecs:[],events:[]});assert.equal(migrated.audit.pass,true);assert.deepEqual(migrated.draftSpecs,[]);
...
document=appendModelExperimentLifecycleEvent(document,draftEvent,{draftSpec:draft});
assert.equal(document.audit.pass,true);assert.equal(document.audit.counts.draft,1);assert.equal(document.audit.effectiveStates[0].effectiveStatus,'DRAFT');assert.deepEqual(document.draftSpecs[0],draft);

const frozen=freezeModelExperimentSpec(draft,{actor:'human-reviewer',approvedAt:'2026-11-17T01:00:00Z'});
assert.equal(frozenSpecMatchesRegisteredDraft(frozen,draft),true);const frozenSnapshot=structuredClone(frozen);const draftSnapshot=structuredClone(document.draftSpecs[0]);const freezeEvent=createFrozenLifecycleEvent(frozen);
document=appendModelExperimentLifecycleEvent(document,freezeEvent,{frozenSpec:frozen});
assert.equal(document.audit.pass,true);assert.deepEqual(document.audit.counts,{draft:0,frozen:1,retired:0});assert.equal(document.audit.effectiveStates[0].effectiveStatus,'FROZEN');assert.deepEqual(document.frozenSpecs[0],frozenSnapshot);assert.deepEqual(document.draftSpecs[0],draftSnapshot);
...
assert.throws(()=>appendModelExperimentLifecycleEvent(noDraft,freezeEvent,{frozenSpec:frozen}),/freeze-without-draft|freeze-draft-snapshot-missing/);
...
const drift=structuredClone(document);drift.frozenSpecs[0].dataset.commitSha='c'.repeat(40);drift.audit=auditModelExperimentLifecycleDocument(drift);assert.equal(drift.audit.pass,false);assert.ok(drift.audit.errorCodes.some(code=>code.includes('fingerprint-mismatch')||code.includes('freeze-dataset-drift')||code.includes('frozen-origin-draft-drift')));

const draftDrift=structuredClone(document);draftDrift.draftSpecs[0].hypothesis='Changed after registration';draftDrift.audit=auditModelExperimentLifecycleDocument(draftDrift);assert.equal(draftDrift.audit.pass,false);assert.ok(draftDrift.audit.errorCodes.some(code=>code.startsWith('draft-fingerprint-drift:')||code.startsWith('frozen-origin-draft-drift:')));
